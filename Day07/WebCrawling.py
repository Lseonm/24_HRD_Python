...
# html = urllib.request.urlopen("https://www.hollys.co.kr/store/korea/korStore2.do?pageNo=47&sido=&gugun=&store=")
# print(html)
#
# soup_hollys = BeautifulSoup(html, "html.parser") # HTML 코드를 가져옴
# print(soup_hollys)
...
for page in range(1, 51):
    ...
    html = urllib.request.urlopen(Hollys_url) # 50개의 url 가져옴

    soup_holiys = BeautifulSoup(html, 'html.parser')
    tag_t_body = soup_holiys.find("tbody") # tbody 찾기
...
Hollys_tbl.to_csv("./Hollys_table.csv", encoding='utf-8', mode='w', index=True)